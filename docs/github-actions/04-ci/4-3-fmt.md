...
`terraform fmt` は、デフォルトでは実行したディレクトリ直下の Terraform ファイルを対象にします。  
サブディレクトリ配下の Terraform ファイルも含めて確認したい場合は、`-recursive` オプションを使います。

フォーマット確認を CI に組み込むことで、レビュー前に「フォーマットが崩れたまま」のコードがメインブランチに混入するのを防げます。

## プラクティス

4-2 で作成した `terraform.yml` に、以下の step を追加してください。
