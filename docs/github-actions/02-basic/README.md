...
- `push` して、GitHub の Actions タブに workflow の実行履歴が表示されること（job は失敗して良い）
...
> 確認：
>
> - `.github/workflows/hello.yml` が作成されている
> - push 後、GitHub の Actions タブに workflow の実行履歴が表示される
...
> 確認：
>
> - workflow が正常に実行され、`Hello, GitHub Actions` が出力される
...
- main 以外のブランチで `push` し、Pull Request を作成して workflow が実行されること
...
> 確認：
>
> - push では workflow が実行されない
> - Pull Request を作成すると workflow が実行される
> - Actions タブでイベントが pull_request になっている
...
> 確認：
>
> - 2つの job が順序指定なしで実行されることを確認する
> - それぞれのログに job1 / job2 が出力される
...
>
> - first job が先に実行される
> - その完了後に second job が実行される
> - 並列ではなく順番に実行されていることが確認できる
